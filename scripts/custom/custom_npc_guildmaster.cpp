...
#define MSG_NOGUILDHOUSE         "Your Guild doesnt have a Guild Hall."
...
bool showBuyList(Player *player, Creature *_creature, uint32 showFromId = 0)
{
	...
	QueryResult *result;
	result = SD2Database.PQuery("SELECT `id`, `comment` FROM `guildhalls` WHERE `guildId` = 0 AND `id` > %u ORDER BY `id` ASC LIMIT %u",
...
bool isPlayerHasGuildhouse(Player *player, Creature *_creature, bool whisper = false)
{
	...
	result = SD2Database.PQuery("SELECT `comment` FROM `guildhalls` WHERE `guildId` = %u",
		player->GetGuildId());
...
void buyGuildhouse(Player *player, Creature *_creature, uint32 guildhouseId)
{
	...
	result = SD2Database.PQuery("SELECT `id` FROM `guildhalls` WHERE `id` = %u AND `guildId` <> 0",
		guildhouseId);
	...
	//update DB
	result = SD2Database.PQuery("UPDATE `guildhalls` SET `guildId` = %u WHERE `id` = %u",
...
void sellGuildhouse(Player *player, Creature *_creature)
{
	if (isPlayerHasGuildhouse(player, _creature))
	{
		...
		result = SD2Database.PQuery("UPDATE `guildhalls` SET `guildId` = 0 WHERE `guildId` = %u",
		player->GetGuildId());
